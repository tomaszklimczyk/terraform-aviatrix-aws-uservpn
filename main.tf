...
resource "aviatrix_spoke_gateway" "ha" {
  ...
  vpc_id             = aviatrix_vpc.default.vpc_id
  account_name       = var.aws_account_name
  subnet             = aviatrix_vpc.default.subnets[length(aviatrix_vpc.default.subnets)-1].cidr
  ha_subnet          = aviatrix_vpc.default.subnets[length(aviatrix_vpc.default.subnets)-2].cidr
...
resource "aviatrix_gateway" "vpn" {
  ...
  subnet           = aviatrix_vpc.default.subnets[length(aviatrix_vpc.default.subnets)-(((count.index % 2) * -1) - 1)].cidr #Using modulo to put even instances in subnet[-1] and odd in subnet[-2]
  vpn_access       = true
